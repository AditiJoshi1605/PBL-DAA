...
def create_graph_from_csv(file_path):
    ...
    print("Missing value counts *before* fix:")
    print(df.isnull().sum())

    # Replace any *remaining* NaN values with a large number representing "no direct connection"
    df = df.fillna(9999)

    #Print info about missing values:
    print("Missing value counts *after* fix:")
    print(df.isnull().sum())
